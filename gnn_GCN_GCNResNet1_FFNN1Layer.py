...
def getConfusion(model, test_loader, reps, hidden_dim, unit1, unit2, loader='test'):
    model.eval()
    loss_fn = nn.CrossEntropyLoss()
    total_loss = 0
    correct = 0
    ...
    avg_loss = total_loss / len(test_loader)
    accuracy = (correct / total) if total > 0 else 0.0
    print(f"{loader.capitalize()} Loss: {avg_loss:.4f}, Accuracy: {accuracy*100:.2f}%")

    # Compute confusion matrix
    cm = confusion_matrix(all_labels, all_preds)
    tn, fp, fn, tp = cm.ravel()
    
    print(reps, hidden_dim, unit1, unit2, loader)
    ...
    for hidden_dim in [32, 64, 128]:
            for numUnit1 in [32, 64, 128, 256, 512, 1024, 2048]:
                ...
                while numUnit2 >= 16:
                    ...
                    torch.save(model, f'GCN_1GCN_1FFNN/model_gnn_GCN_1GCN_1FFNN_{reps}_{hidden_dim}_{numUnit1}_{numUnit2}.pth')

                    tn_train, fp_train, fn_train, tp_train  = getConfusion(model, train_loader, reps, hidden_dim, numUnit1, numUnit2, loader='train')
                    tn_test, fp_test, fn_test, tp_test = getConfusion(model, test_loader, reps, hidden_dim, numUnit1, numUnit2, loader='test')
                    res = {'model' : 'GCN_1GCN_1FFNN',
                           'hidden_dim' : hidden_dim,
                           'numUnit1' : numUnit1,
                           'numUnit2' : numUnit2,
                           'reps' : reps,
...
with open("model_gnn_GCN_1GCN_1FFNN.json", "w") as json_file:
