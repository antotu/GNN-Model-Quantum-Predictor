...
def getConfusion(model, test_loader, reps, hidden_dim, unit1, unit2, unit3, loader='test'):
    model.eval()
    loss_fn = nn.CrossEntropyLoss()
    total_loss = 0
    correct = 0
    ...
    print(reps, hidden_dim, unit1, unit2, unit3, loader)
            ...
            for numUnit1 in [32, 64, 128, 256, 512, 1024, 2048]:
                ...
                while numUnit2 >= 16:
                    ...
                    while numUnit3 >= 16:
                        ...
                        torch.save(model, f'GCN_1GCN_2FFNN/model_gnn_GCN_1GCN_2FFNN_{reps}_{hidden_dim}_{numUnit1}_{numUnit2}_{numUnit3}.pth')

                        tn_train, fp_train, fn_train, tp_train  = getConfusion(model, train_loader, reps, hidden_dim, numUnit1, numUnit2, numUnit3, loader='train')
                        tn_test, fp_test, fn_test, tp_test = getConfusion(model, test_loader, reps, hidden_dim, numUnit1, numUnit2, numUnit3, loader='test')
                        res = {'model' : 'GCN_1GCN_2FFNN',
                            'hidden_dim' : hidden_dim,
                            'numUnit1' : numUnit1,
                            'numUnit2' : numUnit2,
                            'numUnit3' : numUnit3,
...
with open("model_gnn_GCN_1GCN_2FFNN.json", "w") as json_file:
